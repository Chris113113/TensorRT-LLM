# Replaces https://github.com/NVIDIA/TensorRT-LLM/blob/main/benchmarks/cpp/utils/prepare_synthetic_data.py
...
@click.option("--common-prefix-len",
              type=int,
              default=0,
              help="Length of the common prefix to be added to each request.")
@click.pass_obj
def token_norm_dist(root_args, **kwargs):
    ...
    if common_prefix_len > 0:
        ...
        prefix_seed = root_args.random_seed + 1
        common_prefix = gen_random_tokens([common_prefix_len],
                                          root_args.tokenizer,
                                          prefix_seed)[0]
    ...
    if common_prefix:
        input_ids = [common_prefix + ids for ids in input_ids]
        input_lens = [l + common_prefix_len for l in input_lens]
